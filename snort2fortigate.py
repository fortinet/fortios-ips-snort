...
def output_json(outfile, fgt_count, snort_count, output_all):
	...
	if not output_all and fgt_count < 1:
		# don't have any rules to output. abort.
		return False
	out_json = {}
	stats = {}
	stats['success'] = fgt_count
	stats['failure'] = snort_count - fgt_count
	out_json['statistics'] = stats
	results = json.loads(json_stream.getvalue()[:-3] + ']')  # Remove trailing , from stream to close properly
	out_json['results'] = results
	# dumps+write faster than dump
	outfile.write(json.dumps(out_json, ensure_ascii=False, indent=4, sort_keys=True))
	return True
...
def write_sig(rule, sig, sig_name, out_file, gui, j):
	try:
		...
		if not j:
			...
			rule_obj = {}
			rule_obj["original"] = rule
			rule_obj["converted"] = sig
			rule_obj["name"] = sig_name
			rule_obj["messages"] = messages
			rule_obj["success"] = success
...
def main():
	...
	parser.add_argument('--debug', dest='debug', action='store_const', const=True, default=debug_log, help=argparse.SUPPRESS)
	...
	for line in in_f:
		...
		if started_valid_rule:
			...
			# invalid rule from the start, check for -a
			if args.all and line.strip():
				if line.startswith('#'):
					# treat as a comment, ignore
					continue
				if args.json:
					logging.error("Invalid line, unable to parse as Snort rule.")
					write_sig(line, '', '', out_f, args.gui, args.json)
				snort_count += 1
